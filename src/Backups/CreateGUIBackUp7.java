...
public class CreateGUIBackUp7 extends JFrame {
	...
	private int trace_Pos = 0;

	public CreateGUIBackUp7(int trace_Num, int thread_Num, List<Integer> transition_Num, List<List<Integer>> transition_states, List<List<String>> transition_states_info){
		int state = 0;
		int randomR = 0;
		int randomG = 0;
		int randomB = 0;
	   ...
	   // mainPanel.add(scrollingArea, BorderLayout.CENTER);
		...
		final PaintTopGUIBackUp7 topPaint = new PaintTopGUIBackUp7(trace_Num, thread_Num, transition_Num, transition_states, transition_states_info);
		topPaint.setPreferredSize(topScrollDim);
		
        //topPanel.setBackground(Color.white);
        //JScrollPane topScroll = new JScrollPane(topPaint);	
		...
		topPaint.addMouseListener( new MouseAdapter() {
		    public void mousePressed( MouseEvent e ) {
		        trace_Pos = topPaint.getTraceNumber(e.getX(), e.getY());
		        System.out.println("Trace Pos: " + trace_Pos);
		        remove(leftPanel, trace_Pos, leftPanelBtns, transitions);
		        topPaint.repaint();
	...
	public void remove(JPanel leftPanel, int j, List<List<JButton>> leftPanelBtns,  JTextArea scrollText){
		...
		System.out.println("In Remove!! -> " + j );
